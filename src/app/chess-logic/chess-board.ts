...
export class ChessBoard {
            ...
            for(let y = 0; y < this.chessBoardSize; y++) {
                ...
                if(piece instanceof King) {
                    if(this.canCastle(piece, true)) {
                        pieceSafeSqares.push({x, y: 6});
                    }

                    if(this.canCastle(piece, false)) {
                        pieceSafeSqares.push({x, y: 2});
                    }
                }
    ...
    public move(prevX: number, prevY: number, newX: number, newY: number): void {
        ...
        const pieceSafeSqares: Coords[]|undefined = this._safeSquares.get(prevX + "," + prevY);
        if(!pieceSafeSqares || !pieceSafeSqares.find(coords => coords.x === newX && coords.y === newY)) {
